    ...
    <div class="row">
        <div class="col-md-12">
            <section class="panel">
                ...
                <form class="form-horizontal form-bordered" method="POST" enctype="multipart/form-data"  action="{{ route('admin.package.update',['id' => $package->id]) }}">
                    @csrf
                            ...
                            <div class="col-md-6">
                                <select name="category" id="category" class="form-control" required>
                                    ...
                                    @foreach($category as $row)
                                        <option value="{{ $row->id }}" @if($package->category_id == $row->id) {{ 'selected' }} @endif >{{ $row->category }}</option>
                                    @endforeach
                                </select>
