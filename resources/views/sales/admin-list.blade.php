            ...
            <table class="table table-bordered table-striped mb-none" id="datatable-default">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('admin.ticket.view',['id'=> $row->ticketid]) }}">
                                {{ $row->tickettitle }} @if($row->isseen) <span class="pull-right label label-primary">{{ $row->isseen }}</span> @endif
                            </a>
                        </td>
                        <td> 
                        ...
                        <td>
                            <a title="Transaction detail"
                                href="{{ route('admin.view.sales', ['id' => $row->transid]) }}"><button
                                    class="btn btn-primary btn-xs"><i class="fa fa-eye"></i></button>
                            </a>
                            @if($row->transstatus == 'pending')
                            <button title="Transaction delete" id="deleteSales" data-id="{{ $row->transid }}"
...
$(document).ready(function() {
    $("#datatable-default").on("click", "button#deleteSales", function() {
        var id = $(this).data("id");
        swal({
                title: 'Are you sure?',
            ...
            function(willDelete) {
                if (willDelete) {
                    ...
                    var url = '{{ url("/admin/delete/sales") }}' + '/' + id;
                    $.ajax({
                        url: url,
                        type: 'DELETE',
                        data: {
